  ...
  const submitReviewAndUpdateReviews = async event => {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(errorsJSON)) {
        setError(
          Object.entries(errorsJSON).map(([key, values]) => (
            <p key={key} className="block has-text-danger">
              {`${key} ${values.join(' ,')}`}
            </p>
          ))
        )
        ...
        setError(
          <p className="block has-text-danger">
            Unexpected error submitting review, please try again.
          </p>
        )
      }
    }
      ...
      <div className="modal-content">
        <div className="box">
          <form
            ...
            id="new_review"
            acceptCharset="UTF-8"
            onSubmit={submitReviewAndUpdateReviews}
          >
            <label className="title is-4" htmlFor="review_rating">
              Rating
            </label>
                ...
                <Fragment key={i}>
                  <div className={`${i % 1 == 0.5 ? 'first' : 'second'}-half-wrapper`}>
                    <input
                      ...
                      name="review[rating]"
                      id={`review_rating_${i}`}
                      onChange={setRatingWithEvent}
                    />
                    <label
                      className={`star ${i % 1 == 0.5 ? 'first' : 'second'}-half ${
                        i > hoveredRating && 'star-off'
                      }`}
                      htmlFor={`review_rating_${i}`}
                      onMouseEnter={setHoveredRatingWithEvent}
                      onMouseLeave={resetHoveredRatingWithEvent}
                    >
                      ⭐️
                    </label>
                  </div>
            ...
            <label className="title is-4" htmlFor="review_content">
              Review
            </label>
            <div className="block mt-2">
              <textarea
                className={`textarea is-small block ${content.length === 0 && 'is-danger'}`}
                placeholder="Your review"
                name="review[content]"
                id="review_content"
                value={content}
                onChange={setContentWithEvent}
              />
            </div>
            {error}
            <div className="block">
